...
# fedau
python -m puf_unlearning.fedau_main seed=2  dataset="cifar10" alpha=0.3 local_epochs=1 model="ResNet18" fedau.epochs=10 fedau.coefficient=0.04
for (( j=1; j <= recovery_rounds_max; ++j ))
do
  for (( i=0; i <= unl_clients; ++i ))
  do
    echo "$i"
    python -m puf_unlearning.main_puf --multirun seed=2  dataset="cifar10" alpha=0.3 learning_rate=0.1 local_epochs=1 total_rounds=1 resume_training=True unlearned_cid=[$i] resuming_after_unlearning.algorithm="fedau" fedau.epochs=10 fedau.coefficient=0.04 model="ResNet18"
  done
done

# not
python -m puf_unlearning.unlearning_routine dataset="cifar10" alpha=0.3 model="ResNet18" algorithm="not"
for (( j=1; j <= recovery_rounds_max; ++j ))
do
  for (( i=0; i <= unl_clients; ++i ))
  do
    echo "$i"
    python -m puf_unlearning.main_puf --multirun seed=2  dataset="cifar10" alpha=0.3 learning_rate=0.1 local_epochs=1 total_rounds=1 resume_training=True unlearned_cid=[$i] resuming_after_unlearning.algorithm="not" model="ResNet18"
  done
done
